...
export default function DashboardMap({ markers }: { markers: MapMarker[] }) {
  ...
  const handleLocateMe = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (mapRef) mapRef.setView(newCenter, 14, { animate: true });
        setIsLocating(false);
      },
      () => {
        if (mapRef) mapRef.setView(DEFAULT_CENTER, 14, { animate: true });
        setIsLocating(false);
      },
    ...
    <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col h-[450px] relative">
      ...
      <div className="h-14 px-5 border-b border-border flex items-center justify-between shrink-0 bg-white z-10 relative">
        <div>
          ...
          <p className="text-[11px] font-semibold text-slate-400">Monitoring {markers.length} active issues</p>
        </div>
        <div className="flex gap-2">
          <div className="flex items-center gap-1.5 px-2 py-1 bg-red-50 rounded text-[10px] font-bold text-red-700 border border-red-100">
            <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> Critical ({criticalCount})
          </div>
          <div className="flex items-center gap-1.5 px-2 py-1 bg-orange-50 rounded text-[10px] font-bold text-orange-700 border border-orange-100">
            <div className="w-2 h-2 rounded-full bg-orange-500" /> High ({highCount})
          </div>
        </div>
      </div>
        ...
        >
          ...
          {markers.map((marker) => (
            <Marker
              ...
              icon={pins[marker.severity as keyof typeof pins] ?? pins.low}
            >
              <Popup className="custom-popup">
                <div className="p-1 min-w-[150px]">
                  <div className="text-[10px] font-bold text-slate-400 mb-1">{marker.id.slice(0, 8)}</div>
                  <div className="font-bold text-primary mb-2">{formatIssueLabel(marker.yoloClass)}</div>
                  <div className={cn(
